...
func enodeSig() {
	...
	fmt.Printf("\nenodeSig self = \n%s\n\n", enodeJSON.Enode+ethhexutil.Encode(sig))
}
...
func sign() {
	//if *msghash == "" {
	//	*msghash = ethhexutil.Encode(crypto.Keccak256([]byte(*memo)))
	//}
	if len(hashs) == 0 {
		hashs = append(hashs, ethhexutil.Encode(crypto.Keccak256([]byte(*memo))))
	}
...
func signTX(signer types.EIP155Signer, privatekey *ecdsa.PrivateKey, nonce uint64, playload []byte) (string, error) {
	...
	rawTX := ethhexutil.Encode(txdata)
	fmt.Printf("\nSignTx:\nChainId\t\t=%s\nGas\t\t=%d\nGasPrice\t=%s\nNonce\t\t=%d\nToAddr\t\t=%s\nHash\t\t=%s\nData\t\t=%s\n",
